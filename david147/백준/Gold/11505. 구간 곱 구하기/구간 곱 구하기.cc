...
void build(int start, int end, int index) {
    ...
    seg_tree[index] = (seg_tree[2 * index] * seg_tree[2 * index + 1]) % mod;
}
...
long long int interval_product(int start, int end, int index, int left, int right) {
    if (start > right || end < left) return 1;
    if (left <= start && end <= right) return seg_tree[index];

    int mid = (end + start) / 2;
    return (interval_product(start, mid, 2 * index, left, right) * interval_product(mid + 1, end, 2 * index + 1, left, right)) % mod;
}
...
int main()
{
    ...
    for (int i = 0; i < m + h; i++) {
        ...
        else if (a == 2) cout << interval_product(1, n, 1, b, c) << '\n';
    }
